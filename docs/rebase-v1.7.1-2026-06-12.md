...
- Fork tip of record: `origin/main` = `d64264da` (`ci: add GitHub-hosted MindRoom checks`).
...
env -u TUWUNEL_DATABASE_PATH \
  ...
  RUSTC_WRAPPER= \
  PKG_CONFIG_PATH=<liburing-pkgconfig> \
  CFLAGS=-I<liburing-include> \
  CXXFLAGS=-I<liburing-include> \
  LD_LIBRARY_PATH=<liburing-runtime> \
  RUST_TEST_THREADS=1 \
...
- Stop 5: commit `31adcdb0` (`auth: add native Apple login exchange`)
...
- Commit `9948d441` (`auth/apple: address native login review`) applied cleanly, creating `src/api/client/session/sso/native_apple.rs`.
- Stop 6: commit `e9c25c7c` (`Harden SSO grant cookie path`)
  - Conflict: `src/api/client/session/sso.rs` (import lines only).
  - Cause: the preceding native Apple review had moved the route (and its `Json`/`IntoResponse` imports) out to `native_apple.rs`, so this commit's import context disagreed with the post-Stop-5 merged imports; upstream's `CookieJar` (used by `validate_session_cookie`) sat on the same line.
...
- Remaining commits (`Add rebase regression tests`, `ci: add GitHub-hosted MindRoom checks`) applied cleanly; the rebase finished without further stops.
...
- Rebase completed successfully: `main` = `141c295c` (`ci: add GitHub-hosted MindRoom checks`) = upstream `v1.7.1` + the 19 fork commits, linear, no fixups needed.
...
- Rewritten `main` tip: `4183a609` (`ci: add GitHub-hosted MindRoom checks`); diff vs the pre-validation tip is exactly the one `#[allow]` line; `git log --grep='fixup!' v1.7.1..main` is empty; backup refs unmoved.
