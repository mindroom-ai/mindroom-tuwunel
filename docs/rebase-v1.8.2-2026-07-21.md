...
- Fork tip of record: `origin/main` = `efc8b615e`
  (`fix(e2ee): reject replacement of existing device identity keys`).
...
- Fork delta: 12 linear commits, no merges — the six logical commits from the
  ...
  admin deactivation test) and four subsequent feature commits.
...
of the corresponding upstream merge commits confirmed both submissions
landed:

- **DROPPED** `c2729b76c` `fix(e2ee): report drained one-time-key pools`
...
- **DROPPED** `806ef6b9c` `fix(matrixrtc): allow public transport discovery`
  — byte-identical upstream as `a1776c368` (only hunk offsets/context
  differ; `git cherry` missed it because `d16303553` added Matrix v1.18/v1.19
  to the surrounding version list).

Not absorbed, kept: `efc8b615e` (device-key immutability) — upstream's `upload_keys.rs` was
...
`TODO: Remove onetimekeys` the fork resolved. The corresponding change had not
merged as of v1.8.2.
...
- **Dropped OTK follow-on**: upstream `007033cd5` changed the OTK count shape
...
- KEPT (replayed patch-identical) `5112447c2` → sso, `dbf94d222` → apple,
  `34baf29bc` → stream push, `efc8b615e` → device-key immutability.
...
- **UPDATED** docs: this file; FORK_CHANGES.md re-based to v1.8.2, its
  dropped-features note extended, and new sections documenting the
  stream-push and device-key immutability features that landed
...
- Per-commit gates: `cargo check --locked --workspace --all-targets` at every
  code commit of the stack (edits, sso, apple, tests, stream push, device keys = tip-1) — all
...
- `RUST_TEST_THREADS=1 cargo test --locked --workspace --all-targets` at the
  ...
  `tuwunel_service` unit suite (edit_purge, users::sso reactivation, the
...
- Boot smoke on the rebased tip (fresh DB, port 28998): CS API versions 200
  ...
  end-to-end proof the dropped transport-discovery behavior survives via upstream's code — the
...
- The device-key immutability change and its malformed-record and
  device-removal follow-ups were still unmerged at v1.8.2; if the behavior
  lands upstream, drop the fork delta at the next rebase as done for the OTK
  count and transport-discovery changes here.
