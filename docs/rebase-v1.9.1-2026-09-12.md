...
The replacement stack is local on `mindroom/rebase-v1.9.1-verified-20260912`, in an
isolated worktree. No upstream commit or existing release tag is rewritten.
No remote push, release, deployment, or production access is part of this
rebase. Pushing main can trigger release automation and needs a separate
decision.
...
| 2 | Edit lifecycle | Sync compaction, bounded purge, and surviving-edit bundles. |
...
Three negative checks distinguished working composition from a plausible but
incorrect replay:
...
| Edit lifecycle | 1247 | 0 | 4 | Pass / Pass |
| SSO policy | 1261 | 0 | 4 | Pass / Pass |
| Native Apple login | 1278 | 0 | 4 | Pass / Pass |
| Streaming push | 1287 | 0 | 4 | Pass / Pass |
| Device identity | 1288 | 0 | 4 | Pass / Pass |
| CI and releases | 1288 | 0 | 4 | Pass / Pass |
...
Current nightly-2026-09-12 formatting passed the initial runtime tree. Strict
all-target/all-feature Clippy (Rust 1.100.0-nightly) passed the corrected runtime
tree, using the release's own nightly flags from `docker/bake.hcl`. This extra
check found one unnecessary event-ID clone
...
The synthetic upgrade test passes from the saved fork binary identifying as
