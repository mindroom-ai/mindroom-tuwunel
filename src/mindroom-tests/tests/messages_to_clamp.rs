...
mod tests {
	...
	/// position — and covers the `Direction::Forward` arm of the clamp, the
	/// one mindroom-nio's gap-fill hits (dir=f with a sync token as `to`).
	...
	/// An unparseable `to` must be rejected as bad client input — 400
	/// `M_INVALID_PARAM`, like an unparseable `from` — not silently ignored
	/// (the pre-fix behaviour returned HTTP 200 with an unbounded chunk).
	async fn unparseable_to_fails_the_request(router: &Router, room: &str) {
		let (status, body) = request(
			router,
			"GET",
			&format!(
				"/_matrix/client/v3/rooms/{}/messages?dir=b&limit=100&to=garbage!!",
				enc(room)
			),
			Some(ALICE_TOKEN),
			None,
		...
		assert_eq!(
			...
			"unparseable `to` must be a client error, got {status}: {body}"
		);
		assert_eq!(
			body["errcode"], "M_INVALID_PARAM",
			"unparseable `to` must report M_INVALID_PARAM: {body}"
		);
	}
