...
//! (MSC3925).
//!
//! The purge deletes eligible superseded `m.replace` edits, retaining the
//! newest eligible edit per (room, target, sender). State events and invalid
//! or unverifiable replacements are preserved. Upstream bundles the newest
//! surviving edit onto the original at `unsigned.m.relations.m.replace`, from the
