...
mod tests {
	...
	use tuwunel_service::{oauth::Session, users::DeactivationReason};
	use url::Url;
	...
	fn complete_sso_session_reuse_rejection_and_reactivation() -> Result {
		...
		let result = harness.with_services(async |services| {
			...
			// --- Scenario D: one identity is committed serially ----------------
			let identity = "serialized-identity";
			let identity_user = user_id!("@serialized:localhost").to_owned();
			let first_entered = Arc::new(Notify::new());
			let release_first = Arc::new(Notify::new());
			let second_started = Arc::new(Notify::new());
			let second_entered = Arc::new(AtomicBool::new(false));

			let first_sessions = services.oauth.sessions.clone();
			let first_user = identity_user.clone();
			let first_entered_task = first_entered.clone();
			let release_first_task = release_first.clone();
			let first = tokio::spawn(async move {
				first_sessions
					.commit_identity_session(identity, async move |old_user_id| {
						assert!(old_user_id.is_none());
						first_entered_task.notify_one();
						release_first_task.notified().await;

						Ok((
							Session {
								sess_id: Some("serialized-first".to_owned()),
			...
			let second_sessions = services.oauth.sessions.clone();
			let second_user = identity_user.clone();
			...
			let second = tokio::spawn(async move {
				...
				second_sessions
					.commit_identity_session(identity, async move |old_user_id| {
						second_entered_task.store(true, Ordering::Release);
						assert_eq!(old_user_id.as_ref(), Some(&second_user));

						Ok((
							Session {
								sess_id: Some("serialized-second".to_owned()),
								user_id: Some(second_user),
								..Default::default()
							},
							(),
						))
			...
			assert!(
				!second_entered.load(Ordering::Acquire),
				"a second commit for the same identity entered before the first completed",
			);
			...
			assert_eq!(
				services
					.oauth
					.sessions
					.get_sess_id_by_unique_id(identity)
					.await?,
				"serialized-second",
			);
	...
	fn idp_router(sub: &str, email: &str) -> Router {
		...
		Router::new()
			.route(
				...
				any(async |request: Request<Body>| {
					let base = format!(
						"http://{}",
						request
							.headers()
