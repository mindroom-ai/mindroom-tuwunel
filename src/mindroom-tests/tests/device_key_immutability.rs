...
mod tests {
	...
	async fn corrupt_stored_identity_is_not_treated_as_absent(
		...
		// Cover syntax errors as well as invalid UTF-8, which can fail while
		// reading the raw database value, before typed field validation.
		// Neither error may permit replacing a stored identity.
		for invalid_bytes in [b"not-json".as_slice(), b"{".as_slice(), b"\x80".as_slice()] {
			services.db["keyid_key"].put_raw((user_id, device_id), invalid_bytes);
			...
			assert_eq!(
				response.0,
				StatusCode::INTERNAL_SERVER_ERROR,
				"invalid stored bytes must not be overwritten: {}",
				response.1,
			);
			...
			assert_eq!(
				stored.as_ref(),
				invalid_bytes,
				"the failed upload must preserve the original bytes",
			);
		}
