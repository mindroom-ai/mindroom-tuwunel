...
impl crate::Service for Service {
	fn build(args: &crate::Args<'_>) -> Result<Arc<Self>> {
		...
		Ok(Arc::new(Self {
			...
			roomid_ts_pducount: db["roomid_ts_pducount"].clone(),
...
impl Service {
	...
	fn delete_event(&self, candidate: &ReplaceCandidate) -> bool {
		...
		let ts_index_key = match serialize_to_vec((room_id, candidate.origin_server_ts_ms)) {
			| Ok(key) => key,
			| Err(e) => {
				warn!(
					%e,
				...
				.remove_fallible(&ts_index_key)
		{
			warn!(
				...
				room_id = %candidate.room_id,
				ts = candidate.origin_server_ts_ms,
				"Failed to remove superseded edit from roomid_ts_pducount; will retry candidate"
			);
			return false;
		}
...
"#,
	...
	fn assert_event_present(service: &Service, event: &StoredEvent) {
		...
		let ts_index_key = serialize_to_vec((room_id, event.origin_server_ts_ms))
			.expect("serialize timestamp index key");
		let indexed_count = service
			.roomid_ts_pducount
			.get_blocking(&ts_index_key)
			.unwrap_or_else(|_| {
				panic!("expected roomid_ts_pducount entry for {}", event.event_id)
			});
		assert_eq!(
			&*indexed_count,
	...
	fn assert_event_absent(service: &Service, event: &StoredEvent) {
		...
		assert!(
			service
				...
				.get_blocking(&ts_index_key)
				.is_err(),
			"expected no roomid_ts_pducount entry for {}",
			event.event_id,
		);
	}
