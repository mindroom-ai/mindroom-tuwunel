...
impl Service {
	...
	/// index maps media to the events using it, so this scans every stored PDU
	/// in every room and matches any string value in its content equal to a
	/// candidate. Events in `ignored_event_ids` are skipped. A row that does
	/// not decode as a PDU is searched as plain JSON instead, so it can only
	/// add protection; rows that cannot be read at all are counted.
	///
...
"#,
	...
	#[tokio::test]
	async fn purge_preserves_valid_edit_before_newer_malformed_replacement() {
		for new_content in [None, Some(json!(null))] {
			let harness = make_harness(HarnessConfig::default()).await;
			let service = &harness.service;
			let target = insert_event(
				service,
				0,
				"$mixed_target:example.com",
				"@alice:example.com",
				100,
				None,
			);
			let valid = insert_event(
				service,
				1,
				"$mixed_valid:example.com",
				"@alice:example.com",
				200,
				Some(target.event_id.as_str()),
			);
			let malformed = insert_event(
				service,
				2,
				"$mixed_malformed:example.com",
				"@alice:example.com",
				300,
				Some(target.event_id.as_str()),
			);
			update_stored_pdu(service, &malformed, |pdu| {
				let mut content = pdu.get_content_as_value();
				content
					.as_object_mut()
					.expect("content object")
			...
			for _ in 0..2 {
				service
					...
					.expect("purge cycle succeeds");
				assert_event_present(service, &target);
				assert_event_present(service, &valid);
				assert_event_present(service, &malformed);
			}
		}
	}
