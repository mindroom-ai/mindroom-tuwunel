...
mod tests {
	...
	async fn self_deactivated_sso_account_reactivates() {
		...
		assert!(
			!services.users.is_deactivated(user_id).await.expect("deactivation state"),
			"user should no longer be deactivated"
		);
		assert_eq!(
			services.users.origin(user_id).await.expect("origin"),
			"sso",
			"reactivation should preserve SSO origin"
		);
		assert_eq!(
			services.users.password_hash(user_id).await.expect("password hash"),
			PASSWORD_SENTINEL,
...
"#,
		...
		Services::build(server).await.expect("build services")
	}
