...
pub async fn related_events<E: Event>(&self, event: &E) -> Option<Arc<RelatedEvents>> {
	...
	let Ok(content) = event.get_content::<JsonValue>() else {
		return Some(NO_RELATED_EVENTS.clone());
	};

	// Resolve the same content that push rules evaluate, including a terminal
	// replacement's nested reply or thread relation.
	let content = mindroom_terminal_push_content(event.kind(), &content).unwrap_or(content);
	let Ok(ExtractRelatesTo { relates_to }) = serde_json::from_value(content) else {
		return Some(NO_RELATED_EVENTS.clone());
	};
