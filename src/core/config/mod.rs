...
pub struct IdentityProvider {
	...
	/// Additional client IDs whose signed ID tokens should be accepted by native
	/// app login endpoints for this provider. For Sign in with Apple on iOS this
	/// is usually the app bundle identifier, while `client_id` remains the web
	/// Services ID used by the browser SSO flow.
	///
	/// default: []
	#[serde(default)]
