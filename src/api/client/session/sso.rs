...
fn native_apple_provider_id(
	...
	if apple_providers.next().is_some() {
		return Err!(Request(InvalidParam(
			"Native Apple login requires provider_id when multiple AppleOIDC identity providers are configured."
		)));
	}
...
mod tests {
	...
	fn native_apple_provider_id_uses_explicit_provider_id() {
		let providers = [(
			"apple".to_owned(),
			apple_provider_with_native_clients(&["chat.mindroom.app"]),
		)]
		.into();
