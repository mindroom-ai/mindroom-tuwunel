...
`docs/rebase-*.md` for the per-rebase log. As of the 2026-09-05 rebase the base
is upstream `v1.9.0`. Five features this fork originally carried were merged
...
The cleaned v1.9.0 stack puts shared test infrastructure first, keeps each
feature with its tests and compatibility fixes, and ends with CI and docs.
See the [ownership and rebase procedure](docs/rebase-v1.9.0-2026-09-05.md#feature-stack-cleanup-after-the-published-release).
...
- The fork persists the initiator (`self`/`admin`) so *self*-deactivated SSO
  ...
  and the v1.8.2/v1.9.0 windows added no new callers.
...
- Only a genuine not-found result permits insertion. Stored-key decoding or
  ...
  which would bypass the immutability check. Router tests cover malformed JSON
  and invalid UTF-8, asserting that rejected uploads preserve the stored bytes.
