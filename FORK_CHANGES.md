...
`docs/rebase-*.md` for the per-rebase log. As of the 2026-07-07 rebase the base
is upstream `main` at `2323a8ec` (past `v1.8.0`, the first upstream code to carry
native `m.replace` edit bundling, MSC3925). Two features this fork originally
carried — the room-creation default power-level override and the SSO grant-cookie
path hardening — were merged upstream and dropped from the fork as of this
rebase.
...
### 2) `auth/sso: strict-CSP SSO UIAA fallback and self-reactivation`
Files:
...
### 4) `ci: fork release automation, container publishing, and GitHub checks`
Files:
...
Apple, deactivation/erase, edit-purge ↔ bundling composition) so future rebases
catch regressions.
